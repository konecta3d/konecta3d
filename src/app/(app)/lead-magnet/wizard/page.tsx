...
const OBJECTIVE_INFO = {
  captar: { title: "Captar nuevos clientes", description: "Atrae prospectos interesados en tu negocio", example: "Guia: 'Como elegir el mejor servicio'", color: "#10B981" },
  volvieron: { title: "Que vuelvan", description: "Fideliza y haz que tus clientes compren de nuevo", example: "Checklist: 'Guia de mantenimiento'", color: "#3B82F6" },
  conversion: { title: "Vender mas", description: "Aumenta tus ventas o conversiones", example: "Recomendacion: 'Productos perfectos'", color: "#F59E0B" },
  referidos: { title: "Invitar referidos", description: "Que tus clientes traigan nuevos clientes", example: "Guia: 'Comparte y gana'", color: "#8B5CF6" }
};
...
// Opciones comunes para botones CTA
const CTA_PRESETS = [
  "Descargar ahora",
  "Reservar cita",
  "Contactar",
...
function LeadMagnetWizardInner() {
  ...
  const MAX_CONTENT_CHARS = 1200; // Limite para caber en una pagina
  ...
  const getTypeLabel = () => {
    const labels: Record<LeadMagnetType, string> = { guia: "GUIA ESTRATEGICA", checklist: "CHECKLIST DE IMPLEMENTACION", recomendacion: "RECOMENDACION TECNICA" };
    return labels[type];
  };
  ...
  const renderContentPreview = () => {
    if (type === "checklist") {
        ...
        <ul style={{ listStyle: "none", padding: 0 }}>
          {customContent.split("\n").filter(l => l.trim()).map((l, i) => (
            <li key={i} style={{ display: "flex", alignItems: "flex-start", gap: "10px", marginBottom: "0.8rem" }}>
              <span style={{ minWidth: "18px", height: "18px", border: `2px solid ${colorButton}`, borderRadius: "4px", display: "inline-block", marginTop: "3px" }}></span>
              <span style={{ color: "#e5e7eb" }}>{l}</span>
            </li>
          ))}
        </ul>
      );
      ...
      return (
        <ol style={{ paddingLeft: "1.5rem", color: "#e5e7eb", listStyle: "decimal" }}>
          {customContent.split("\n").filter(l => l.trim()).map((l, i) => (
            <li key={i} style={{ marginBottom: "1rem", paddingLeft: "10px", color: "#e5e7eb", listStylePosition: "inside" }}>
              {l}
            </li>
          ))}
        </ol>
    ...
    return <div style={{ whiteSpace: "pre-line", color: "#e5e7eb", lineHeight: 1.8 }}>{customContent}</div>;
  };
        ...
        return (
          <div className="space-y-8">
            <div className="text-center">
              <h2 className="text-3xl font-bold mb-4" style={{ color: "#ffffff" }}>Convierte tu conocimiento en un activo para tu negocio</h2>
            ...
            <div className="bg-gradient-to-r from-[#0a323c] to-[#001e3c] rounded-xl p-6 border border-[#39a1a9]/30">
              <h3 className="text-lg font-bold mb-3" style={{ color: "#ffffff" }}>Que vas a crear?</h3>
              ...
              <ul className="text-sm text-slate-200 space-y-2">
                <li className="flex items-center gap-2">
                  <span className="text-[#ffb400]">✓</span> Guia con pasos clave de tu servicio
                </li>
                <li className="flex items-center gap-2">
                  <span className="text-[#ffb400]">✓</span> Checklist de verificacion para tus clientes
                </li>
                <li className="flex items-center gap-2">
                  <span className="text-[#ffb400]">✓</span> Recomendaciones profesionales exclusivas
                </li>
              </ul>
            </div>
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-8">
              <h2 className="text-2xl font-bold mb-2" style={{ color: "#ffffff" }}>Cual es tu objetivo?</h2>
              <p className="text-slate-200">Selecciona el proposito de tu lead magnet</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(OBJECTIVE_INFO).map(([key, info]) => (
                <button key={key} onClick={() => setObjective(key as Objective)} className="p-6 rounded-xl text-left transition-all border-2" style={{ borderColor: objective === key ? info.color : "rgba(255,255,255,0.1)", backgroundColor: objective === key ? `${info.color}15` : "rgba(255,255,255,0.05)" }}>
                  <div className="flex items-center gap-3 mb-3"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: info.color }}></div><h3 className="text-lg font-bold" style={{ color: "#ffffff" }}>{info.title}</h3></div>
                  <p className="text-sm text-slate-200 mb-3">{info.description}</p>
                  <div className="text-xs text-slate-300 italic">Ej: {info.example}</div>
                </button>
              ))}
            </div>
            <div className="flex justify-end mt-8"><button onClick={() => setStep("tipo")} className="px-8 py-3 rounded-full bg-[#ffb400] text-black font-bold">Continuar</button></div>
          </div>
        );
      case "tipo":
        return (
          <div className="space-y-6">
            <div className="text-center mb-8"><h2 className="text-2xl font-bold mb-2" style={{ color: "#ffffff" }}>Que tipo de recurso es?</h2><p className="text-slate-200">Elige el formato</p></div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {Object.entries(TYPE_INFO).map(([key, info]) => (
                <button key={key} onClick={() => setType(key as LeadMagnetType)} className="p-6 rounded-xl text-center transition-all border-2" style={{ borderColor: type === key ? "#ffb400" : "rgba(255,255,255,0.1)", backgroundColor: type === key ? "rgba(255,180,0,0.1)" : "rgba(255,255,255,0.05)" }}>
                  <h3 className="text-lg font-bold mb-2" style={{ color: "#ffffff" }}>{info.title}</h3><p className="text-sm text-slate-200 mb-3">{info.description}</p><div className="text-xs text-slate-300 italic">Ej: {info.example}</div>
                </button>
              ))}
            </div>
            ...
            <div className="flex justify-between mt-8"><button onClick={() => setStep("objetivo")} className="px-6 py-3 rounded-full border border-white/20" style={{ color: "#ffffff" }}>Atras</button><button onClick={() => setStep("contenido")} className="px-8 py-3 rounded-full bg-[#ffb400] text-black font-bold">Continuar</button></div>
          </div>
        );
      case "contenido":
        // Parse content into individual points (remove numbering like "1.", "2.", etc.)
        const contentPoints = customContent.split("\n").filter(l => l.trim());
        const parsedPoints = contentPoints.map((point, idx) => {
          // Remove leading numbers and dots like "1.", "2.", "1)", "2)", etc.
          return point.replace(/^(\d+[\.\)])\s*/, "").replace(/^[-•]\s*/, "");
        });
        
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h2 className="text-2xl font-bold mb-2" style={{ color: "#ffffff" }}>Personaliza tu contenido</h2><p className="text-slate-200">Edita los datos generados</p></div>
            ...
            <div className="space-y-4 mb-6">
              ...
              {type === "guia" ? (
                <div>
                  ...
                  <textarea
                    ...
                    style={{ color: "#ffffff" }}
                  />
                  <div className={`text-xs mt-2 ${customContent.length > MAX_CONTENT_CHARS * 0.9 ? 'text-red-400' : 'text-slate-300'}`}>
                    {customContent.length} / {MAX_CONTENT_CHARS} caracteres (maximo para 1 pagina)
                  </div>
                </div>
              ) : (
                <div>
                  <label className="block text-xs uppercase tracking-widest text-[#39a1a9] mb-2">Contenido - Puntos (seleccionables)</label>

                  {/* Entrada rápida para nuevos puntos */}
                  <div className="flex gap-2 mb-3">
                    <input
                      type="text"
                      value={newPoint}
                      onChange={(e) => setNewPoint(e.target.value)}
                      className="flex-1 px-3 py-2 rounded-lg bg-white/10 border border-white/10 text-white text-sm placeholder-slate-400"
                      style={{ color: "#f9fafb" }}
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto bg-white/5 rounded-lg p-3 border border-white/10">
                    {parsedPoints.length === 0 ? (
                      <p className="text-slate-200 text-sm">No hay puntos disponibles</p>
                    ) : (
              ...
              <div className="border-t border-white/10 pt-4"><label className="block text-xs uppercase tracking-widest text-[#39a1a9] mb-2">Cierre / Llamada a la accion</label>
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-3 mt-8"><button onClick={() => setStep("tipo")} className="w-full sm:w-auto px-6 py-3 rounded-full border border-white/20" style={{ color: "#ffffff" }}>Atras</button><button onClick={() => setStep("personalizacion")} className="w-full sm:w-auto px-8 py-3 rounded-full bg-[#ffb400] text-black font-bold">Continuar</button></div>
          </div>
        );
      case "personalizacion":
        return (
          <div className="space-y-6">
            <div className="text-center mb-6"><h2 className="text-2xl font-bold mb-2" style={{ color: "#ffffff" }}>Personalizacion de botones y colores</h2><p className="text-slate-200">Configura los botones CTA y los colores del documento</p></div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="bg-white/5 rounded-xl p-4">
                <div className="flex items-center justify-between mb-4"><h3 className="text-white font-bold">Boton Principal (CTA 1)</h3><label className="flex items-center gap-2 text-sm text-slate-300"><input type="checkbox" checked={cta1Enabled} onChange={(e) => setCta1Enabled(e.target.checked)} className="w-4 h-4 accent-[#ffb400]" />Activar</label></div>
                {cta1Enabled && <div className="space-y-3">
                  <div>
                    <label className="block text-xs text-slate-300 mb-1">Texto del boton</label>
                    <div className="flex gap-2">
                      ...
                      >
                        ...
                        {CTA_PRESETS.map(opt => <option key={opt} value={opt} style={{ color: "white", backgroundColor: "#1e293b" }}>{opt}</option>)}
                      </select>
                    </div>
                    <input
                      type="text"
                  ...
                  <div><label className="block text-xs text-slate-300 mb-1">Enlace (URL)</label>
                    <div className="space-y-2">
                      <ActionLinkPicker 
                        value={cta1Link} 
                        onChange={setCta1Link}
                        label=""
                      />
                      <input type="text" value={cta1Link} onChange={(e) => setCta1Link(e.target.value)} placeholder="O escribe una URL manualmente..." className="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/10 text-white text-sm" /></div>
                  </div>
                </div>}
              </div>
              <div className="bg-white/5 rounded-xl p-4">
                <div className="flex items-center justify-between mb-4"><h3 className="text-white font-bold">Boton Secundario (CTA 2)</h3><label className="flex items-center gap-2 text-sm text-slate-300"><input type="checkbox" checked={cta2Enabled} onChange={(e) => setCta2Enabled(e.target.checked)} className="w-4 h-4 accent-[#ffb400]" />Activar</label></div>
                {cta2Enabled && <div className="space-y-3">
                  <div>
                    <label className="block text-xs text-gray-400 mb-1">Texto del boton</label>
                    <div className="flex gap-2">
                      <select 
                        value={cta2Text} 
                  ...
                  <div><label className="block text-xs text-slate-300 mb-1">Enlace (URL)</label>
                    <div className="space-y-2">
                      <ActionLinkPicker 
                        value={cta2Link} 
                        onChange={setCta2Link}
                        label=""
                      />
                      <input type="text" value={cta2Link} onChange={(e) => setCta2Link(e.target.value)} placeholder="O escribe una URL manualmente..." className="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/10 text-white text-sm" /></div>
                  </div>
                </div>}
              </div>
            </div>
            ...
            <div className="bg-white/5 rounded-xl p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                  <label className="block text-xs text-slate-300 mb-2">Color marca (texto nombre negocio, borde)</label>
                ...
                {/* Color etiqueta */}
                <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                  <label className="block text-xs text-slate-300 mb-2">Color etiqueta (fondo badge)</label>
                  <div className="flex items-center gap-3">
                ...
                {/* Color titulo */}
                <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                  <label className="block text-xs text-slate-300 mb-2">Color titulo</label>
                  <div className="flex items-center gap-3">
                ...
                {/* Color boton */}
                <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                  <label className="block text-xs text-slate-300 mb-2">Color boton (CTA principal)</label>
                  <div className="flex items-center gap-3">
                ...
                <div className="bg-white/5 rounded-lg p-4 border border-white/10 md:col-span-2">
                  <label className="block text-xs text-slate-300 mb-2">Tamano del titulo</label>
                  ...
                  {/* Tamaño del logo */}
                  <label className="block text-xs text-slate-300 mb-2">Tamano del logo</label>
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-3 mt-8"><button onClick={() => setStep("contenido")} className="w-full sm:w-auto px-6 py-3 rounded-full border border-white/20" style={{ color: "#ffffff" }}>Atras</button><button onClick={savePdf} disabled={pdfGenerating} className="w-full sm:w-auto px-8 py-3 rounded-full bg-[#ffb400] text-black font-bold">{pdfGenerating ? "Guardando..." : "Guardar PDF"}</button></div>
          </div>
        );
    }
  };

  const steps = ["bienvenida", "objetivo", "tipo", "contenido", "personalizacion"];
    ...
    <div className="min-h-screen bg-[#0f1720] p-4 md:p-6 lg:p-8 overflow-x-auto">
      <div className="max-w-[1400px] mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-8">
          <div>
            <h1 className="text-[#ffb400] text-base md:text-lg font-extrabold tracking-widest uppercase">Asistente Lead Magnet</h1>
            <p className="text-slate-200 text-xs md:text-sm">Convierte tu conocimiento en un recurso valioso</p>
          </div>
          <Link href="/lead-magnet/new" className="text-slate-300 hover:text-white text-xs md:text-sm">
          ...
          {steps.map((s, i) => (
            <div key={s} className="flex items-center">
              <button
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${step === s ? "bg-[#ffb400] text-black" : steps.indexOf(step) > i ? "bg-green-500 text-white" : "bg-white/10 text-gray-400 hover:bg-white/20"}`}
              >
                {i + 1}
              </button>
              {i < 4 && (
        ...
        <div className="bg-[#0f1720]/50 rounded-2xl border border-white/10 p-6 md:p-10">{renderStep()}</div>
      </div>
    </div>
  );
}
