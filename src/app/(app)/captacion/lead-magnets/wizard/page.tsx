...
function CaptacionLeadMagnetWizardInner() {
      ...
      if (editId) {
        ...
        if (lm) {
          ...
          if (c && typeof c === "object") {
            ...
            if (typeof c.contentSize === "number") setContentSize(c.contentSize);
            if (typeof c.contentLineHeight === "number") setContentLineHeight(c.contentLineHeight);
            if (typeof c.btnSize === "number") setBtnSize(c.btnSize);
            if (typeof c.btnRadius === "number") setBtnRadius(c.btnRadius);
            if (typeof c.btnShadow === "boolean") setBtnShadow(c.btnShadow);
            if (typeof c.btnArrow === "boolean") setBtnArrow(c.btnArrow);
  ...
  const buildContent = () => ({
    ...
    subtitleSize, contentSize, contentLineHeight,
    btnSize, btnRadius, btnShadow, btnArrow,
      ...
      case "personalizacion":
        return (
          <div className="space-y-6">
            ...
            <div className="bg-[var(--card)] rounded-xl p-4 mb-4 border border-[var(--border)]">
              <h3 className="text-[var(--foreground)] font-bold text-sm mb-3">Estilo de los botones</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-[var(--foreground)]/50 mb-2">Tamaño del botón ({btnSize.toFixed(2)})</label>
                  <input type="range" min="0.7" max="1.4" step="0.05" value={btnSize} onChange={e => setBtnSize(Number(e.target.value))} className="w-full" />
                </div>
                <div>
                  <label className="block text-xs text-[var(--foreground)]/50 mb-2">Forma</label>
                  <select value={btnRadius} onChange={e => setBtnRadius(Number(e.target.value))} className="w-full px-3 py-2 rounded-lg border text-[var(--foreground)] text-sm" style={{ background: "var(--card)", borderColor: "var(--border)" }}>
                ...
                <label className="flex items-center gap-2 text-xs text-[var(--foreground)]/70">
                  <input type="checkbox" checked={btnShadow} onChange={e => setBtnShadow(e.target.checked)} className="w-4 h-4 accent-[var(--brand-4)]" />Sombra en el botón principal
                </label>
                <label className="flex items-center gap-2 text-xs text-[var(--foreground)]/70">
                  <input type="checkbox" checked={btnArrow} onChange={e => setBtnArrow(e.target.checked)} className="w-4 h-4 accent-[var(--brand-4)]" />Flecha → en el botón principal
                </label>
              </div>
            </div>
            ...
            <div className="bg-[var(--card)] rounded-xl p-4 md:p-6 mb-4 border border-[var(--border)]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                ...
                <div className="bg-[var(--card)] rounded-lg p-3 border border-[var(--border)]">
                  <label className="block text-xs text-[var(--foreground)]/50 mb-2">Tamaño del subtítulo ({subtitleSize}rem)</label>
                  <input type="range" min="0.7" max="1.8" step="0.05" value={subtitleSize} onChange={e => setSubtitleSize(Number(e.target.value))} className="w-full" />
                </div>
                <div className="bg-[var(--card)] rounded-lg p-3 border border-[var(--border)]">
                  <label className="block text-xs text-[var(--foreground)]/50 mb-2">Tamaño del contenido ({contentSize}rem)</label>
                  <input type="range" min="0.6" max="1.4" step="0.05" value={contentSize} onChange={e => setContentSize(Number(e.target.value))} className="w-full" />
                </div>
                <div className="bg-[var(--card)] rounded-lg p-3 border border-[var(--border)]">
                  <label className="block text-xs text-[var(--foreground)]/50 mb-2">Interlineado ({contentLineHeight})</label>
                  <input type="range" min="1" max="2.2" step="0.1" value={contentLineHeight} onChange={e => setContentLineHeight(Number(e.target.value))} className="w-full" />
                </div>
              </div>
            </div>
