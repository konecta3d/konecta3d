...
<body>
  <div class="header">
    <div class="header-label">Mi Contexto — Konecta3D</div>
...
export default function MiContextoPage() {
  ...
  // ── GPT URL (solo si module_gpt activo) ──────────────────────────────────
  useEffect(() => {
    if (!moduleGpt) return;
    const loadSettings = async () => {
      const { data } = await supabase.from("settings").select("value").eq("key", "gpt_url").single();
      if (data?.value) {
        try {
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      const filename = `mi-contexto-${businessName.toLowerCase().replace(/\s+/g, "-")}.pdf`;
      ...
      if (data.error) { alert("Error al generar el PDF: " + data.error); return; }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-16">
      ...
      <div className="flex items-center justify-between gap-4 pt-2">
        <div>
          ...
          <p className="text-sm text-[var(--foreground)]/60 mt-1">
            La identidad de tu negocio que alimenta todos los asistentes IA
          </p>
        </div>
        <Link
      ...
      {/* Banner de bienvenida — solo cuando está vacío */}
      {isEmpty && (
        <div className="rounded-xl border border-[var(--brand-1)]/30 bg-[var(--brand-1)]/8 p-5">
      ...
      {/* Banner de progreso parcial */}
      {!isEmpty && !isComplete && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/8 px-4 py-3 flex items-center gap-3">
          <span className="text-amber-400 text-lg">⚠</span>
          <p className="text-sm text-[var(--foreground)]/80">
            Tienes <strong>{totalCount - answeredCount} {totalCount - answeredCount === 1 ? "pregunta" : "preguntas"} sin responder</strong>. Los asistentes funcionan mejor con el perfil completo.
          </p>
        </div>
      )}
      ...
      {isComplete && (
        <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/8 px-4 py-3 flex items-center gap-3">
          ...
          <p className="text-sm text-[var(--foreground)]/80">
            <strong>Perfil completo.</strong> Todos los asistentes IA están personalizados para tu negocio.
          </p>
        </div>
      )}
        ...
        <div className="h-2 rounded-full bg-[var(--border)] overflow-hidden">
          <div
            ...
            style={{
              ...
              backgroundColor: isComplete ? "var(--brand-3)" : "var(--brand-1)",
            }}
          />
        </div>
        {saveStatus === "saving" && (
      ...
      <div className="space-y-4">
          ...
          <div key={q.id} className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
            <label className="block text-sm font-semibold mb-2">
              <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold mr-2 ${
                ...
                {answers[q.id]?.trim() ? "✓" : q.question_order}
              </span>
              {q.question_text}
            </label>
            <textarea
        ...
        {notesQuestion && (
          <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
            <label className="block text-sm font-semibold mb-2">
              <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold mr-2 ${
                answers[notesQuestion.id]?.trim()
                  ? "bg-emerald-500/15 text-emerald-400"
                  : "bg-[var(--brand-1)]/15 text-[var(--brand-1)]"
              }`}>
                {answers[notesQuestion.id]?.trim() ? "✓" : notesQuestion.question_order}
              </span>
            ...
            <textarea
              ...
              placeholder="Cualquier información adicional que quieras que conozcan los asistentes..."
              className="w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-2.5 text-sm resize-y focus:outline-none focus:ring-2 focus:ring-[var(--brand-1)]/40 transition"
            />
          </div>
        )}
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          {/* GPT externo — solo si el módulo está activo */}
          {moduleGpt && (
            <a
              href={gptUrl}
              target="_blank"
        ...
        {moduleGpt && (
          <p className="text-xs text-[var(--foreground)]/40 text-center mt-3">
            Consejo: copia el texto y pégalo en el GPT al inicio de cada conversación para mejores respuestas
          </p>
        )}
      </div>
    </div>
